...
const QuizQuestions: React.FC<QuizQuestionsProps> = ({ quizId }) => {
	...
	return (
		<div className="container mx-auto mt-5">
			<div className="">
				<div className="ml-64 fixed top-1/5 left-0 w-60 bg-white text-dark p-6 rounded-r-lg shadow-lg">
					<div className="flex justify-start mb-5">
						<Button onClick={handleBack}>Back</Button>
					</div>
					<h3 className="text-xl font-semibold mb-2">
						{quiz.title ? quiz.title : "No Quiz Title"}
					</h3>
					<ul>
							...
							quiz.questions.map((question, index) => (
								<li key={index} className="mb-2">
									<a
										...
										className={`p-1 ${activeIndex === index ? "border rounded bg-blue-50 w-100" : ""}`}
										onClick={(e) => {
											e.preventDefault();
				...
				<div className="ml-64 p-6">
					<h1 className="text-center text-3xl font-bold">
						...
						&gt;{" "}
						<Link href={`/vakken/quiz/quiz-type/${course.id}`}>
						...
						{quiz.questions && quiz.questions.length > 0 ? (
							quiz.questions.map((question, index) => (
								<div
									...
									className="card mb-3 border rounded-lg shadow-sm"
								>
